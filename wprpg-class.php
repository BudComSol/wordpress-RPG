...
class wpRPG {
    ...
    public function wpRPG_check_tables() {
        ...
        $wpdb->query($sql);
		$sql = "CREATE TABLE IF NOT EXISTS `wprpg_rpg_player_metas` (
																  id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    public function checkUserMeta($id) {
        $default_usermeta = $this->setDefaultUserMetas();
	...
	public function setDefaultUserMetas() {
		global $wpdb;
		$default = array();
		...
		{
			if(is_admin()){
			add_option( 'wpRPG_default_'.$row['name'] , ($row['value']=='time'?time():$row['value']), '', 'yes' );
			register_setting( 'rpg_settings', 'wpRPG_default_'.$row['name']);
			}
			array_push($default, array($row['name']=>$row['value']));
		}
		return $default;
	}
